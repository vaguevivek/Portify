...
const AccordionItem: React.FC<{
    ...
    return (
        <div className="border-b border-gray-100 last:border-0">
            ...
            >
                <span className={`text-lg font-medium transition-colors duration-300 ${isOpen ? 'text-indigo-600' : 'text-gray-900 group-hover:text-indigo-600'}`}>
                    {item.question}
                </span>
                <span className={`ml-6 flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${isOpen ? 'bg-indigo-100 text-indigo-600 rotate-180' : 'bg-gray-50 text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'}`}>
            ...
            >
                <p className="text-gray-600 leading-relaxed text-base pr-8">
                    {item.answer}
                </p>
            </div>
        </div>
...
const Faq: React.FC = () => {
    ...
    return (
        <section id="faqs" className="bg-white py-20 sm:py-24 relative overflow-hidden">
             {/* Subtle background decoration */}
             <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full pointer-events-none">
                <div className="absolute top-[20%] right-[-10%] w-96 h-96 bg-indigo-50 rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
                <div className="absolute bottom-[10%] left-[-5%] w-72 h-72 bg-purple-50 rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
            </div>
            ...
            <div className="container mx-auto px-9 relative z-10">
                ...
                <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-2 sm:p-8">
                ...
                <div className="mt-12 text-center">
                    <p className="text-gray-500">
                        Still have questions? <a href="#" className="text-indigo-600 font-semibold hover:text-indigo-700 transition-colors">Contact our support team</a>
                    </p>
                </div>
            </div>
        </section>
